...
```{r get_data}
sobj <- readRDS(file.path("objects", "sobj_v2.rds"))
```

```{r, fig.width = 12, fig.height = 12}

malignant_cells <- colnames(sobj)[sobj$cell_phenotype == "Malignant"]
...
so <- readRDS(file.path("objects", "malignant_sobj.rds"))
```
...
so_sub <- subset(so, subset = RNA_snn_res.0.05 == "0")
FindMarkers(so_sub, ident.1 = "GP4", ident.2 = "SHH", group.by = "subgroup")
```
