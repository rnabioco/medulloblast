...
plot_harmony(so, "coarse_clusters_gp3_harmony")
```
...
mkrs %>% 
  ...
  split(., .$group) %>% 
  write_markers_xlsx(.,
                     file.path(tbls_dir, "harmony_markers_gp3_only.xlsx"))
```
...
GetAssayData(so, "counts") %>% 
  ...
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  data.table::fwrite(., file.path(tbls_dir, "gp3_count_matrix.tsv.gz"),
       sep = "\t",
       compress = "gzip")
...
expr_mat %>% 
  ...
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  data.table::fwrite(., file.path(tbls_dir, "gp3_expr_matrix.tsv.gz"),
       sep = "\t",
       compress = "gzip")
...
go_res[["result"]] %>% 
  mutate(parents = unlist(map(parents, 
                              ~str_c(.x, collapse = ",")))) %>% 
  write_tsv(file.path(tbls_dir, "goterms_gp3_clusters.tsv"))
...
plot_harmony(so, "tumor_cell_type_gp3")
```
...
so_mdata <- get_metadata(so) %>% 
  select(cell, tumor_cell_type_gp3)

so_neo <- qread(file.path("objects", "preprocess", "all_neoplastic.qs"))

new_mdata <- get_metadata(so_neo) %>% 
  select(cell, tumor_cell_types_all_subgroups, tumor_cell_type_subgroup_specific) %>% 
...
mkrs %>% 
  ...
  split(., .$group) %>% 
  write_markers_xlsx(.,
                     file.path(tbls_dir, "harmony_markers_gp4_only.xlsx"))
```
...
GetAssayData(so, "counts") %>% 
  ...
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  data.table::fwrite(., file.path(tbls_dir, "gp4_count_matrix.tsv.gz"),
       sep = "\t",
       compress = "gzip")
...
expr_mat %>% 
  ...
  as.data.frame() %>% 
  rownames_to_column("gene") %>% 
  data.table::fwrite(., file.path(tbls_dir, "gp4_expr_matrix.tsv.gz"),
       sep = "\t",
       compress = "gzip")
...
go_res[["result"]] %>% 
  mutate(parents = unlist(map(parents, 
                              ~str_c(.x, collapse = ",")))) %>% 
  write_tsv(file.path(tbls_dir, "goterms_gp4_clusters.tsv"))
...
plot_harmony(so, "tumor_cell_type_gp4")
```
...
```{r}
so_mdata <- get_metadata(so) %>% 
  select(cell, tumor_cell_type_gp4)
