...
mkrs <- read_tsv("markers/aligned_cluster_markers_all_data.tsv") %>% 
...
```{bash}
cd cellbrowser
echo -e '\ncolors="/Users/kriemo/Projects/sc_repos/medulloblast/results/cellbrowser/alldata_colorMap.csv"' >> medulloblastoma/cellbrowser.conf
```
...
cols <- colnames(sobj@meta.data)
names(cols) <- colnames(sobj@meta.data)
mkrs <- read_tsv("markers/res_0.02_malignant_cluster_markers.tsv") %>% 
  select(cluster, gene, p_val_adj, everything())
...
```{bash}
cd cellbrowser
echo -e '\ncolors="/Users/kriemo/Projects/sc_repos/medulloblast/results/cellbrowser/malignant_colorMap.csv"' >> medulloblastoma_malignant/cellbrowser.conf
```


## Normal and immune populations
