...
```{r}
so <- readRDS(file.path(obj_dir,"shh", "shh_model_sobj.rds"))
...
plot_umap(so, "malignant_clusters")

mkrs <- wilcoxauc(so, "malignant_clusters")
...
mkrs %>% 
  write_tsv(file.path(mkrs_dir, "shh", 
...
mkrs %>% 
  ...
  split(., .$group) %>% 
  write_markers_xlsx(.,
                     file.path(tbls_dir, "shh",
                               "shh_neoplastic_cluster_markers.xlsx"))
...
```{r}
h_so <- readRDS(file.path("objects","shh","shh.rds"))
```

Get Variable genes
```{r}
mouse_var_genes <- FindVariableFeatures(so, 
                           selection.method = "vst", 
                           nfeatures = 3000, 
                           verbose = FALSE) %>% 
  VariableFeatures() %>% 
  tibble(mmusculus_homolog_associated_gene_name = .) %>% 
  left_join(.,
            orthologs,
            by = "mmusculus_homolog_associated_gene_name") %>% 
  pull(ortho_id) %>% 
  na.omit() %>% 
  unique()

human_var_genes <- VariableFeatures(h_so) %>% 
    tibble(external_gene_name = .) %>% 
  left_join(.,
            orthologs,
...
var_genes <- union(mouse_var_genes, human_var_genes)



# try marker genes
m_mkrs <- filter(mkrs, logFC > 0, padj < 0.05) %>% 
  group_by(group) %>% 
  arrange(padj, desc(logFC), .by_group = TRUE) %>% 
  dplyr::slice(1:500) %>% 
  pull(feature) %>% 
...
h_mkrs <- read_tsv(file.path("markers", "shh", "harmony_markers_shh.tsv")) %>% 
...
var_genes <- union(m_mkrs, h_mkrs)
```


```{r}
mats <- set_shared_orthologs(h_so, so, orthologs)
