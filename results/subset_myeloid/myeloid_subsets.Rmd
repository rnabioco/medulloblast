...
myeloid_cells <- colnames(sobj)[sobj$RNA_snn_res.0.05 %in% c(0, 5)]

sobj <- sobj[, myeloid_cells]

sobj@reductions$umap_merged <- NULL
...
sobj$clusters_unaligned <- sobj$RNA_snn_res.0.3
```
...
outname <- file.path("tables", paste0("myeloid_metadata_", format(Sys.Date(), "%Y_%m_%d"), ".tsv.gz"))
...
```{r}
saveRDS(sobj, file.path("objects", "myeloid_sobj.rds"))
```

## markers

```{r}
sobj <- readRDS(file.path("objects", "myeloid_sobj.rds"))
Idents(sobj) <- "clusters_unaligned"

mkrs <- FindAllMarkers(sobj, only.pos = TRUE)

write_tsv(mkrs, file.path(mkrs_dir, "res_0.05_unaligned_myeloid_cluster_markers.tsv"))
```

```{r}
markers <- read_tsv(file.path(mkrs_dir, "res_0.05_unaligned_myeloid_cluster_markers.tsv"))
...
openxlsx::write.xlsx(c(readme_sheet, 
                       list_of_markers),
                     "markers/myeloid_cluster_markers_unaligned.xlsx")
...
map_dfr(res, 
        ~tibble(clusterName = names(.x), 
                color = .x)) %>% 
  as.data.frame() %>% 
  write_csv("cellbrowser/myeloid_colorMap.csv", col_names = F, quote_escape = "none")
...
mkrs <- read_tsv("markers/res_0.05_unaligned_myeloid_cluster_markers.tsv") %>% 
  select(cluster, gene, p_val_adj, everything())

write_tsv(mkrs, "cellbrowser/markers/myeloid_cluster_markers.tsv")

do.call(function(...) {ExportToCellbrowser(sobj, 
                    dir = file.path("cellbrowser",
                                    "medulloblastoma_myeloid"),
                    dataset.name = "medulloblastoma_myeloid",
                    #reductions = c("umap_unaligned", "umap_aligned"),
                    reductions = c("umap_unaligned"),
                    markers.file = "cellbrowser/markers/myeloid_cluster_markers.tsv",
...
```{bash}
cd cellbrowser
echo -e '\ncolors="/Users/kriemo/Projects/sc_repos/medulloblast/results/subset_myeloid/cellbrowser/myeloid_colorMap.csv"' >> medulloblastoma_myeloid/cellbrowser.conf
```
