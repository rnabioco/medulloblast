...
openxlsx::write.xlsx(top_diff_mkrs, file.path(tbls_dir,
...
openxlsx::write.xlsx(top_undiff_mkrs, file.path(tbls_dir,
...
go_res[["result"]] %>% 
  ...
  select(-significant) %>% 
  split(., .$query) %>% 
  openxlsx::write.xlsx(., file.path(tbls_dir, "goterms_undifferentiated_cell_types.xlsx"))
...
go_res[["result"]] %>% 
  ...
  select(-significant) %>% 
  split(., .$query) %>% 
  openxlsx::write.xlsx(., file.path(tbls_dir, "goterms_differentiated_cell_types.xlsx"))
```
...
tmp <- subset(so, subset = tumor_cell_type %in% diff_pops)
tmp@assays$TF <- NULL
...
tmp <- subset(so, subset = tumor_cell_type %in% undiff_pops)
tmp@assays$TF <- NULL
tmp <- ScaleData(tmp, features = mkrs)
