...
library(harmony)
so <- RunHarmony(so, "species", theta = 1.0) %>% 
...
plot_umap(so, 
          c("subgroup", "model", "species", "subpopulation"), 
          .cols = c(palette_OkabeIto[-4], discrete_palette_default),
          sorted = "random",
          legend_title = "") 
```
...
subgroups <- unique(so$subgroup)
h <- Embeddings(so, "harmony")

avg_h <- lapply(subgroups, 
       function(x){
         ids <- names(so$subgroup)[so$subgroup == x]
         h_tmp <- h[ids, ]
         m <- colMeans(h_tmp)
...
dend <- as.dendrogram(hclust(dist(t(avg_h)), method = "ward.D2"))
...
save_plot("tst.pdf", p, base_asp = 0.75, base_height = 8)
