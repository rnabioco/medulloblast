...
# subcluser the fibroblasts
fibro_clusters <- subset(so, subset = publication_cell_types == "Fibroblast") %>% 
  FindNeighbors() %>% 
  FindClusters(resolution = c(0.2)) %>% 
  .@meta.data %>% 
  rownames_to_column("cell_id") %>% 
  select(cell_id, cluster = `RNA_snn_res.0.2`) 

fibro_clusters <- mutate(fibro_clusters,
                       new_publication_cell_types = ifelse(cluster == "0", 
                                                       "Fibroblast",
                                                       "Vasc.endothelium"))

so <- so@meta.data %>% 
  rownames_to_column("cell_id") %>%
  select(cell_id, publication_cell_types) %>% 
  left_join(fibro_clusters, by = "cell_id") %>% 
  mutate(publication_cell_types = ifelse(publication_cell_types == "Fibroblast",
                                         new_publication_cell_types,
...
p <- plot_umap(so,
          ...
          legend_title = "",
          label_color = "black",
          ggrepel_opts = list(force = 0.001,
                              point.padding = 0.25))

p

save_plot(file.path(fig_dir, "umap_dnp53_cell_type_labels.pdf"),
          p,
          base_asp = 1.5)
```
...
mkrs <- get_marker_summaries(so, 
                             "publication_cell_types",
                             prefix = "myc_dnp53_pub_cell_types",
                     tsv_output_dir = mkrs_dir,
                     xlsx_output_dir = tbls_dir)
mkrs
...
mkrs <- get_marker_summaries(so_sub, 
                             "publication_cell_types",
                             prefix = "myc_dnp53_pub_cell_types_malignant",
                     tsv_output_dir = mkrs_dir,
                     xlsx_output_dir = tbls_dir)
rm(mkrs)
```
...
mouse_markers <- read_tsv(file.path("markers", "mouse", "gp34", "myc_dnp53_pub_cell_types_malignant_cluster_markers.tsv")) %>% 
filter(padj < 0.05) %>% 
  group_by(group) %>% 
  arrange(padj, desc(logFC), .by_group = TRUE) %>% 
