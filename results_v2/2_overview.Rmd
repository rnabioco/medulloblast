...
p <- plot_umap(so, "hpca_cell_types", label_text = TRUE, label_size = 3.) +
  theme(legend.position = "none")
...
plt_dat <- so@meta.data %>% 
  ...
  group_by(subgroup, fine_cell_type_ids) %>% 
  summarize(percent_of_cells = 100 * n() / unique(total_cells)) %>% 
  spread(subgroup, percent_of_cells, fill = 0L)

p <- plt_dat %>% 
  gather(subgroup, percent, -fine_cell_type_ids) %>% 
  ggplot(aes(fine_cell_type_ids, percent)) +
