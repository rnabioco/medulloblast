...
write_tsv(file.path(mkrs_dir, "subgroup_markers_neoplastic.tsv"))
...
filter(mkrs, 
  ...
  group_by(group) %>% 
  arrange(padj, desc(logFC)) %>% 
write_tsv(file.path(mkrs_dir, "upn_markers_neoplastic.tsv"))
...
filter(mkrs, 
  ...
  group_by(group) %>% 
  arrange(padj, desc(logFC)) %>% 
write_tsv(file.path(mkrs_dir, "upn_markers_neoplastic_pos_and_neg.tsv"))
